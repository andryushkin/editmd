...
- **Fragment replacement в Preview сохраняет редактирование plugin-card**: persistent shell переносит active control и selection в новый DOM; Swift перед заменой сбрасывает native editable-focus gate, а успешный restore возвращает его через `focusin`.
