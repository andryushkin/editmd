...
- **формулы-спринт** (версия не поднималась) — LaTeX-формулы `$…$`/`$$…$$` через все три режима: `MathScan.swift` (сканер + маска U+E000, код пропускается, валютные гарды), Preview = KaTeX-рендер офлайн (`Resources/katex/` — js + css с data-URI-шрифтами, вшивается в страницу только при наличии формул; `KaTeXResources.swift`), Source = `.mathBody`/`.mathMarker` в `collectSpans`, Visual = `.mdMath`-раны (вербатим TeX с видимыми `$`, сериализация без эскейпа — чинит `\frac`→`\\frac`), кнопка ∫ в strip = меню вставки inline/block шаблона. `markdownHTMLRender` возвращает `(body, hasMath)`; demo-formulas.md в корне
...
- **Формулы: Preview парсит МАСКИРОВАННЫЙ текст** — math-спаны заменяются U+E000 юнит-в-юнит до cmark (`MathScan.swift`), UTF-16-раскладка и переводы строк сохраняются → все оффсеты валидны в оригинале; `LineIndex` строить от маскированного текста, не оригинала. Visual: `.mdMath`-раны (вербатим TeX, Int-атрибут) сериализуются БЕЗ `escapeInline` — эскейп калечил бы `\frac`/`_` (формулы-спринт).
...
└── EditMD/
    ...
    EditMDTests/
        ...
        └── MathScanTests.swift             # 18 кейсов: сканер формул (inline/display, гарды валюты/эскейпов, пропуск кода) + маска
