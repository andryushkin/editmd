...
- **Sidecar меток — формат smotr as-is** (`*.review.json`, `rev` + merge by id, якоря `quote+prefix`); unknown-поля в `extra`-бэге, round-trip без потерь. Схему не расширять без согласования (v37).
