...
#   2  the run did not happen (no dictionary, no audit base, a broken grep)
...
check_patterns() { # check_patterns <file> <section-name>
    while IFS= read -r pat; do
        printf '' | grep -E -e "$pat" > /dev/null 2>&1
        [ $? -le 1 ] || die2 "not a valid pattern in $2: $pat"
    done < "$1"
}
check_patterns "$work/deny" publicity-dictionary
check_patterns "$work/allow" publicity-allow

# The allow list is applied by BLANKING every permitted spelling out of the text
# before the dictionary is searched. That implements "a hit lying wholly inside
# a permitted spelling is not a hit" without carrying offsets around: what
# survives the blanking is exactly the text no permission covers. A denied word
# that merely overlaps a permitted one still survives it and is still reported.
: > "$work/scrub.sed"
d=$(printf '\001')   # a delimiter no pattern of ours can contain
while IFS= read -r pat; do
    printf 's%s%s%s %sg\n' "$d" "$pat" "$d" "$d" >> "$work/scrub.sed"
done < "$work/allow"
...
collect_diff() { # collect_diff <label>   (a diff arrives on stdin)
    awk -v label="$1" '
        /^\+\+\+ / { path = substr($0, 7)
                     if (path != "/dev/null") printf "%s %s\tpath: %s\n", label, path, path
                     next }
...
sed -E -f "$work/scrub.sed" "$work/text" > "$work/scrubbed" || die2 "blanking the allow list failed"

hits=$(grep -n -E -f "$work/deny" "$work/scrubbed")
st=$?
[ "$st" -le 1 ] || die2 "grep errored ($st) over the collected text"
[ "$st" -eq 0 ] || exit 0

# Report against the ORIGINAL line, not the blanked one: the blanked line is
# what the decision was made on, but it is not what the author wrote. The entry
# that fired is named too — "something was found" is not actionable, and the
# author needs to know which word to stop writing.
for n in $(printf '%s\n' "$hits" | cut -d: -f1); do
    line=$(sed -n "${n}p" "$work/stream")
    scrubbed=$(sed -n "${n}p" "$work/scrubbed")
    where=${line%%	*}
    text=${line#*	}
    while IFS= read -r pat; do
        if printf '%s\n' "$scrubbed" | grep -q -E -e "$pat"; then
            printf '%s: matches /%s/\n' "$where" "$pat"
            printf '      %s\n' "$text"
            break
        fi
