...
В Visual та же ошибка показывается оранжевым chip `Multi-checkbox · Needs attention` в статус-баре. Tooltip содержит конкретную причину, а клик переключает документ в Source и ставит позицию на начало frontmatter. Для валидной конфигурации chip не показывается.
