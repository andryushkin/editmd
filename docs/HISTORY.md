...
- **Лента тянется вместе с панелью — и это работа системного контрола, а не наша.** Жалобы «кнопки не растягиваются вслед за сайдбаром» и «у активной кнопки нет разделителей по бокам» я сначала чинил в самописной ленте (flex-фреймы вместо фиксированных, `Capsule` вместо `Circle`, гашение двух хайрлайнов вокруг выделения). Всё это — штатное поведение `Picker` + `.pickerStyle(.segmented)` (то есть `NSSegmentedControl`), который заодно даёт системный вид, анимацию, клавиатуру и accessibility. Итог: `SidebarNavStrip` = сегментированный пикер, `SidebarNavTabButton` / `SidebarNavDivider` / кастомная капсула-фон удалены. **Мораль:** прежде чем рисовать поведение стандартного контрола руками, проверь, нет ли его в SwiftUI.
  - **Цена перехода:** точка-бейдж на табе Review невозможна (сегмент — один `Image`), поэтому при `openCount > 0` подставляется filled-вариант символа (`SidebarNavTab.badgeSystemImage`); индивидуальные тултипы `editMDHelp` на иконках пропали — остался `accessibilityLabel` на сегменте.
