...
- **Пол выводится из метрик ленты**, а не из числа: `SidebarChrome.navigatorPillWidth(tabs:)` + `2 × barPaddingH` (квадратные 24pt-ячейки + pad капсулы; hairline — overlay и в бюджет не входит). Добавится таб — пол поедет сам.
- **Лента — кастомная Xcode-капсула, не `NSSegmentedControl`.** Нужны: flex-табы на всю ширину панели, выделение-`Capsule` (круг на floor width, овал при растягивании), хайрлайны что гаснут по бокам active, точка-бейдж на Review, per-tab tooltip. SwiftUI `Picker` + `.segmented` не заполняет ширину и не даёт tooltips; «голый» `NSSegmentedControl` через representable заполняет, но selection chrome — прямоугольный bezel, не круг→овал как в Xcode. Итог: `SidebarNavStrip` / `SidebarNavTabButton` / `SidebarNavDivider` на SwiftUI, выделение = `Capsule(style: .continuous)` с `maxWidth: .infinity`.
  - **Тупик, в который не надо возвращаться:** системный segmented (SwiftUI или AppKit) — другой visual language. Для navigator strip Xcode-look рисуется руками.
