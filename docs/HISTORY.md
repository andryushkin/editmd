...
- **Лента тянется вместе с панелью** (канон — навигатор Xcode): табы получили `minWidth: iconButtonWidth` + `maxWidth: .infinity` вместо фиксированной ширины, а хвостовой `Spacer` убран — слак делится между кнопками поровну. Выделение рисуется `Capsule`, а не `Circle`: на полу ширины это тот же круг, шире — пилюля. Бейдж «открытых меток» вешается на иконку **до** растягивающего `.frame`, иначе на широкой панели точка уезжала к дальнему краю кнопки. Формула пола не изменилась — минимум кнопки как раз `iconButtonWidth`. Дублировавшийся код кнопки и разделителя сведён в `SidebarNavTabButton` / `SidebarNavDivider`.
