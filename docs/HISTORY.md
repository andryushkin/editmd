...
- Ревью-фиксы temы typora (после codex-ревью 83b807a): (1) dark-блок переделан из перекраски Github в порт Night (выше); (2) Open Sans реально бандлится — woff 400/400i/700/700i из той же репы Typora лежат в `Resources/opensans/` (Apache 2.0, `opensans-LICENSE.txt` + запись в THIRD_PARTY_NOTICES.md) и встраиваются в CSS темы data:-URI (CSP страницы разрешает только `font-src data:` — внешний URL не загрузился бы; unicode-range латиница как у Typora, кириллица честно падает в Helvetica Neue); (3) референсная геометрия 16px/860 из github.css. Гоча ресурсов: сборка сплющивает подпапки `Resources/*` в корень bundle — файлы ищутся subdirectory→flat (как KaTeX), а имена ресурсов должны нести префикс (`opensans-400.woff`, не `400.woff`).
- Follow-up ревью (3bdb4fb): resolve-на-рендере для геометрии оказался ловушкой — «15/736» неотличимо от явного выбора, Settings и action strip показывали одно, Preview рисовал другое. Решение: **геометрия пишется в реальные настройки один раз при смене темы** — `EditorSettings.migratedPreviewGeometry(current, from:, to:)` в didSet `previewTypography` переписывает fontSize/columnWidth только пока они равны эффективным дефолтам УХОДЯЩЕЙ темы (иначе — не трогает; «0 = полная ширина» всегда пользовательский выбор). Рендер-пути (Preview, PDF, strip) читают сырые настройки — один источник правды, `resolved*`-хелперы удалены. Довершён ритм Night в dark-блоке: p/ul/ol/table `margin: 0 0 1.5rem` (night.css:242), pre `0 0 20px` (fences), цитата низ 1.875rem, таблица `display: table; width: 100%` — узкие таблицы растягиваются, широкие переносят текст в ячейках вместо скролла, ровно как в Typora Night (display:block базы в тёмной typora сознательно перекрыт).
- Follow-up 2 (4a9dcb3): didSet не срабатывает в init — установка, выбравшая typora на билде с render-time resolver'ом, грузилась бы с сохранёнными 15/736 навсегда. Добавлена versioned startup-миграция по образцу `visualTypographyBaseline`: stamp `previewGeometryBaseline` (=1) + `legacyPreviewGeometryUpgrade(stored, activeThemeID:)` = `migratedPreviewGeometry(from: .standard)` — стоковые значения трактуются как нетронутые, что воспроизводит ровно то, что те версии РИСОВАЛИ (resolver показывал 16/860 при сохранённых 15/736; «осознанные 15/736 под typora» до selection-time записи не существовали — их резолвили в 16/860). После первого запуска stamp защищает уже осознанный выбор стоковых чисел от повторной перезаписи. Explicit `persist` + запись stamp'а в init (didSet молчит).
...
## Generation gate для ActiveInlineFormats (2026-07-23)

Закрыт пункт техдолга из todo: все три режима доставляют форматы каретки в стрип через async-хоп (Source/Visual — `DispatchQueue.main.async`, Preview — очередь сообщений WKWebView), и publish, посчитанный до переключения режима, мог приземлиться ПОСЛЕ сброса `activeFormats` в `setEditorMode` и заново зажечь устаревшие флаги. У Source/Visual стоял частичный guard (`textView?.window != nil`), у Preview/WebKit-пути не было никакого.

Решение — `ActiveFormatsGate` (`Views/ActiveFormatsGate.swift`): epoch-счётчик в `@State`-боксе `ContentView`; каждый sink штампуется эпохой на момент рендера, `setEditorMode` делает `advance()` и тем самым отзывает все sink'и уходящего редактора. File epoch отдельного счётчика не требует: `.id(url)` пересоздаёт `ContentView` и деаллоцирует гейт, weak-ссылка в sink'е роняет поздние publish'и сама. Старый window-guard в координаторах оставлен как ремень. Тесты — `ActiveFormatsGateTests` (доставка/отзыв/новая эпоха/деаллокация).

**Follow-up по внешнему ревью:** control-путь (`editmdctl mode …`, `ControlRouter.setMode`) пишет `editorMode` в UserDefaults напрямую, мимо `setEditorMode` — advance и сброс не выполнялись. Фикс двумя слоями, оба срабатывают для ЛЮБОГО писателя shared-дефолта: (1) `ActiveFormatsGate.noteMode(mode)` вызывается в начале `editorArea` строго ДО построения sink'ов этого прохода и бампает эпоху при смене режима — порядок «advance, потом sinks» гарантирован конструкцией, тогда как `.onChange`-бамп не гарантирует порядок относительно body детей и мог бы отозвать свежие sink'и; (2) идемпотентный хвост перехода (`modeDidChange`: commit typing, сброс `activeFormats`, закрытие Preview-⌘F, чистка table ops) перевешен с `setEditorMode` на `.onChange(of: storedMode)`. В `setEditorMode` остался только синхронный belt (commit + advance + reset до записи дефолта): publish, уже лежащий в main queue, может исполниться раньше SwiftUI-коммита, а `modeDidChange` подчищает только в коммите. Гонка control-пути закрыта той же парой: «ранний» stale-publish затирается сбросом в коммите, «поздний» роняется эпохой.

## Подготовка к публикации репозитория (2026-07-23)

Аудит перед переводом GitHub-репо в public: секретов в файлах и всей истории (472 коммита) нет, git-идентичность — обычная авторская. Сделано:

- **`docs/typora/` удалена.** Репозиторий typora/typora-default-themes не имеет лицензии, значит прав на редистрибуцию скопированных `github.css`/`night.css`, HTML-экспорта живой Typora и скриншотов её UI не было. Сам порт темы в `PreviewTheme.swift` легален (дизайн-параметры не копирайтятся), шрифты Open Sans — тоже (у шрифта своя лицензия Apache 2.0). По решению пользователя история затем вычищена `git filter-repo --path docs/typora --invert-paths` (2026-07-23, до первого public push): файлов нет ни в одном коммите, SHA коммитов с 2026-07-22 переписаны (упоминания в этом файле обновлены по commit-map), бэкап-mirror до перезаписи — `~/Server/editmd-backup-pre-filter.git`.
- **Тема переименована для пользователя: «Typora» → «Night»** (ru «Ночная»). Внутренний id `"typora"` сохранён: он записан в настройках пользователей и по нему ключуется legacy-upgrade геометрии (`previewGeometryBaseline`).
- **Гитлинки `research/CodeEdit` и `research/MarkEdit` удалены из индекса** (вложенные репо без `.gitmodules` — у клонирующих были бы битые пустые папки); `research/` добавлена в `.gitignore`.
- Добавлены `LICENSE` (MIT), корневой `README.md` (EN), в `THIRD_PARTY_NOTICES.md` дописаны KaTeX (vendored!), swift-markdown, swift-cmark, SwiftMath.
- Из `AGENTS.md` убрана ссылка на давно удалённый `visual-audit.md`.
