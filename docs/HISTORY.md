...
- **Перемещение файлов (2026-07-15).** Файл можно перетащить на корень workspace, подпапку или открытую карточку папки либо вызвать «Переместить…» и выбрать каталог. Обе UI-дороги используют одну асинхронную дисковую операцию: существующий destination не перезаписывается, открытый документ блокирует move, `.review.json` следует за файлом с откатом основного move при ошибке, а hidden/loose/pinned, last-active, sidebar snapshot, history, tags и wiki-link index получают новый URL только после успеха. Относительные markdown-ссылки внутри переносимого документа автоматически не переписываются.
