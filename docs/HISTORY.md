...
- **11.5 Таблицы.** Общие токены native/island: в покое только горизонтальные линии (0.5pt между строками, 1pt под header), никаких вертикалей и рамки; native — по-рёберные ширины `NSTextBlock.setWidth(...edge: .maxY)`; island показывает колоночные сепараторы только при hover острова (`hoveredIslandLocation`) или row-drag. Заливка header убрана в обоих. Паддинг ячейки 8 (native padding, island pad + measure-константы 16). Табличные цифры: `blockKind == .tableCell` даёт `monospacedDigitSystemFont` (кастомная семья пользователя — без подмены), island берёт те же шрифты. Floor высоты строки ячейки = body 1.3 → покойная строка ~34–36pt.
...
**Follow-up по глазам (2026-07-22):** пользователь вернул полную сетку таблиц — 0.5pt hairlines везде (вертикали и рамка, как линии строк), 1pt под header, заливки header по-прежнему нет. Hover-only вертикали island и `hoveredIslandLocation` убраны как ненужные.
