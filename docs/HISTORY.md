...
- **Перемещение файлов (2026-07-15).** Файл можно перетащить на корень workspace, подпапку или открытую карточку папки либо вызвать «Переместить…» и выбрать каталог. Обе UI-дороги используют одну асинхронную операцию: существующий destination не перезаписывается, `.review.json` следует за файлом с откатом основного move при ошибке, а hidden/loose/pinned, last-active, sidebar snapshot, history, tags и wiki-link index получают новый URL только после успеха. Открытый Markdown сначала flush-ится и паркуется в `DocumentRegistry` session cache, main/lite-представления закрываются и после move восстанавливаются по новому URL с той же моделью и undo; у активного review сначала дожидаемся FIFO sidecar pipeline. Общий `LongRunningOperationCenter` сразу блокирует ввод, но показывает material-overlay с `ProgressView` только после 250 мс, поэтому быстрые операции не мигают; слой подключён ко всем main/lite/settings roots и поддерживает перекрывающиеся задачи. Относительные markdown-ссылки внутри переносимого документа автоматически не переписываются.
