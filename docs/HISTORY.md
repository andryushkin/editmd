...
- **Мышь.** Боковые кнопки (buttonNumber 3=back, 4=forward) через локальный `NSEvent`-монитор в `AppDelegate`; съедаются только когда key-окно документное (`representedURL != nil`), нав no-op'ит на краях.
- **Инвариант.** Browser-семантика (новый переход обрезает forward-хвост), relocate/discardPaths при перемещении файлов работают по `entries`. Тест-конструктор `DocumentHistory(observingWindows:)` даёт изолированный инстанс без window-observer; suite `Document history Back/Forward` покрывает стек, dedup, обрезание хвоста и штамповку/восстановление оффсета.
