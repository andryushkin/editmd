...
- **Visual**: парсит ТОЖЕ маскированный текст (как Preview) — `=`-строка внутри `$$` больше не делает блок setext-H1, `\\`/`\,` не разэскейпливаются. Однострочные матчи в `appendTextWithWikiLinks` слиты с wiki-матчами (сорт по позиции, при оверлапе побеждает ранний); math-ран = **вербатим source-слайс с видимыми `$`** + атрибут `.mdMath` (Int 0/1 — дёшево хешируется, v29). Многострочный `$$`-блок перехватывается на уровне блоков (`multilineMathIndex`): весь спан — один вербатим-ран с U+2028 вместо `\n` (модель параграфов цела), моно-шрифт; blank-line-осколки спана молча пропускаются; сентинелы без локального матча (lazy-continuation) показывают source-слайс. Сериализатор эмитит `.mdMath`-раны без эскейпа (U+2028 → `\n`) — чинит и доспринтовую порчу `$\frac{a}{b}$` → `$\\frac{a}{b}$`. Презентация тонирует как inline-code + мono-шрифт (без фоновой плашки). Кнопка ∫ (function) в strip — меню «Встроенная/Блочная формула»: вставка `$…$`/`$$…$$` с `.mdMath` и выделением TeX-тела (заглушка-алерт удалена).
