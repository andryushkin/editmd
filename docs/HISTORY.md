...
Мотив: правый инспектор перетаскивался до 150pt, а его пилюля навигатора требует 264 — хвостовые табы (Links / Backlinks / Info) уезжали под край панели. У левого сайдбара тот же дефект в миниатюре: пол 150 против 159 на четыре таба, подрезался Tags.

- **Пол выводится из метрик ленты**, а не из числа: `SidebarChrome.navigatorPillWidth(tabs:)` + `2 × barPaddingH`, а сама пилюля рисуется теми же константами (`navPillPaddingH` / `navDividerWidth` / `navDividerPaddingH`). Добавится таб — пол поедет сам.
- **Лента тянется вместе с панелью — и это работа системного контрола, а не наша.** Жалобы «кнопки не растягиваются вслед за сайдбаром» и «у активной кнопки нет разделителей по бокам» я сначала чинил в самописной ленте (flex-фреймы вместо фиксированных, `Capsule` вместо `Circle`, гашение двух хайрлайнов вокруг выделения). Всё это — штатное поведение `NSSegmentedControl`, который заодно даёт системный вид, анимацию, клавиатуру и accessibility. Итог: `SidebarNavStrip` = сегментированный контрол, `SidebarNavTabButton` / `SidebarNavDivider` / кастомная капсула-фон удалены. **Мораль:** прежде чем рисовать поведение стандартного контрола руками, проверь, нет ли его в системе.
  - **SwiftUI `Picker` + `.pickerStyle(.segmented)` не подошёл** — промежуточная версия выглядела мимо канона: пикер держит intrinsic-ширину и центрируется в панели вместо заполнения (`segmentDistribution` из SwiftUI недоступен), да и per-segment тултипов не даёт. Поэтому `SidebarNavStrip` — `NSViewRepresentable` над `NSSegmentedControl` с `.fillEqually`, `controlSize = .large`, низким hugging по горизонтали и `sizeThatFits`, возвращающим предложенную ширину.
  - **Цена перехода:** точка-бейдж на табе Review невозможна (сегмент несёт один `NSImage`), поэтому при `openCount > 0` подставляется filled-вариант символа (`SidebarNavTab.badgeSystemImage`); счётчик открытых меток переехал в тултип сегмента.
  - **Пол ширины не пересчитывался** (`navigatorPillWidth` и цифры 264/159 в `PaneLayoutTests` те же): сегментированный контрол при нехватке места сжимается, а не подрезает хвостовые табы, но иконки становятся нечитаемыми — прежний бюджет остаётся разумным минимумом. Константы `navPillPaddingH` / `navDividerWidth` / `navDividerPaddingH` больше ничего не рисуют и живут только внутри этой формулы.
