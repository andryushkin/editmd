...
## Открытые вопросы (решить на ревью плана)

1. `.editmd/link-index.json` в git: коммитить (граф доступен агентам после
   clone, диффы осмысленные) или gitignore (mtime-поля шумят в диффах)?
   Возможный компромисс: два файла — `graph.json` (без mtime, коммитится) +
   `cache.json` (mtime, gitignore), ценой дублирования. Рекомендация: один
   файл, решение per-vault за владельцем.
2. Нужна ли команда чтения «сырого» индексного файла агентом через editmdctl
   (`index.dump`), или чтение файла напрямую достаточно?
3. Персистить ли индексы НЕ активных workspace при переключении (scanCache
   их уже держит в памяти) — или писать только активный?
