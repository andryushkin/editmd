...
## Проверка после ревью (2026-07-17, 99b6157 + 4791f1f)

Нативный /code-review спринта дал 10 находок; все исправлены (см. коммиты).
Сверка с планом — реализовано всё, кроме осознанных отклонений:

- **Этап 6**: tools `get_selection` и `open_diff` в editmd-mcp не реализованы —
  `open_diff` сознательно остаётся в /ide (блокирующий continuation живёт в
  app); `get_selection` и авто-регистрация MCP в `~/.codex/config.toml` /
  `.mcp.json` — follow-up.
- **Этап 3**: «единый источник» промптов — фактически два синхронизируемых
  вручную (код `AgentPromptCatalog` + `prompts.md` в skill-пакете); при правке
  одного обновляй второй.
- **Этап 2**: установка хуков идемпотентна и переустанавливаема, но кнопки
  удаления (undo) нет — follow-up.
- **Этап 5**: «статусы одним словарём строк» не делалось — chips в MarkCard
  остались локальными (уже локализованы); общий `statusLabel` удалён как
  неиспользуемый.
- **Этап 7**: auto-apply чистого буфера был и остаётся безусловным (инвариант
  v34); настройка управляет только тостом, лейбл приведён в соответствие.
